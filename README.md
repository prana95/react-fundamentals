...
#### Suspense and use
React 19 introduced a new way to work with asynchronous operations such as fetching data from an API.

Async Opeation
- From React 19 onwards: use
- Not a hook
- Pass in a promise, retuens result
- component suspended until result available

There's now a function available simply called use. Important to understand is that this is, despite its name, not a hook. It's just a function. So the rules of hooks don't apply here. It can be used anywhere inside the components function. Use expects a promise as a parameter, and it will return a result of the promise when it resolves. While the promise is in an unresolved state, which basically means it's still doing its work, the component will be put in a suspended state. The rendering will be paused until the promise resolves.
